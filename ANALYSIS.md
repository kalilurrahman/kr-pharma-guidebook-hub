...
- **Dependency vulnerabilities — the safe work is now exhausted; everything left needs a breaking major.** As of the latest audit, **13 advisories remain (2 moderate, 11 high)** and *none* can be remediated without a major upgrade. Verified individually rather than inferred from npm's summary, because `fixAvailable: true` is misleading — it means "a fix exists somewhere", not "applicable without breaking changes":
  - **`react-router` / `react-router-dom` (moderate ×2)** — patched only in **≥ 7.18.0**. The latest 6.x is **6.30.4**, which the project already has, so there is *no fix inside the 6.x line*. Remediation means a **React Router 6 → 7 migration** — a real API change affecting all nine routes. `npm audit fix` correctly refuses it. (A third advisory on this package, "open redirect leading to XSS", *is* already fixed by 6.30.4.)
  - **The ESLint chain** (`eslint`, `@eslint/config-array`, `@eslint/eslintrc`, `minimatch`, `brace-expansion`) — all resolve only via **`eslint@10`** (major). Dev-only tooling.
  - **The PWA/workbox chain** (`vite-plugin-pwa`, `workbox-build`, `ejs`, `jake`, `filelist`, `@trickfilm400/rollup-plugin-off-main-thread`) — npm proposes `vite-plugin-pwa@1.2.0`, which is **older** than the installed 1.3.0 and would break Vite 8 compatibility. Not actionable as advised.
  **Recommendation:** treat React Router 7 and ESLint 10 as two separate, deliberately scoped migrations. Do not paper over them with `--force` or `--legacy-peer-deps`; that is what produced the un-installable lockfile described below.

- **Guard the gate with branch protection.** The Vite 5 → 8 upgrade was merged despite its own PR title saying *"DRAFT — do not merge without a deploy preview"*, and it broke `main`: its lockfile only resolved under `--force`/`--legacy-peer-deps`, so a clean `npm ci` failed and every CI step after install was skipped. Fixed by bringing three plugins up to Vite-8-compatible versions (`@vitejs/plugin-react-swc` 4.3.2, `vite-plugin-pwa` 1.3.0, `lovable-tagger` 1.3.3). The CI gate now exists but **is not enforced** — requiring the check on `main` would have stopped this.

- **Dependabot backlog is mostly stale.** Of 19 open PRs, **14 are redundant** — `main` already carries an equal or newer version (e.g. `vite` 8.1.5 > PR #12's 8.0.8; `vitest` 3.2.7 > #25's 3.2.6; `rollup` 4.62.2 > #1's 4.60.0). Dependabot normally closes these itself on its next scan; they can also be closed in bulk. Only PR #20 (`brace-expansion`) tracks something still present, and it is part of the ESLint-10 chain above.

- **Earlier dependency work, for context — partially fixed.** A non-breaking `npm audit fix` (lockfile-only, `package.json` untouched) took the tree from **26 → 14** advisories and eliminated the **critical** one. Notably it patched `react-router-dom` 6.30.1 → 6.30.4 — the only *browser-shipped* runtime package in the set; the rest are build/dev toolchain.
